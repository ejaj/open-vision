...
face_model = cv2.dnn.readNetFromCaffe(
    'data/faces_data/detection/deploy.prototxt',
    'data/faces_data/detection/res10_300x300_ssd_iter_140000.caffemodel')
...
age_model = cv2.dnn.readNetFromCaffe(
    'data/faces_data/age_gender_classification/age_net_deploy.prototxt',
    'data/faces_data/age_gender_classification/age_net.caffemodel')
age_labels = ['0-2', '4-6', '8-12', '15-20',
              '25-32', '38-43', '48-53', '60+']

gender_model = cv2.dnn.readNetFromCaffe(
    'data/faces_data/age_gender_classification/gender_net_deploy.prototxt',
    'data/faces_data/age_gender_classification/gender_net.caffemodel')

gender_labels = ['Male', 'Female']

age_gender_blob_size = (256, 256)
age_gender_average_image = np.load(
    'data/faces_data/age_gender_classification/average_face.npy')
...
while ret:
    ...
    for face in face_output[0, 0]:
        ...
        if face_confidence > face_confidence_threshold:
            ...
            age_output = age_model.forward()
            age_id = np.argmax(age_output)
            age_label = age_labels[age_id]
            age_confidence = age_output[0, age_id]

            gender_model.setInput(age_gender_blob)
            gender_output = gender_model.forward()
            gender_id = np.argmax(gender_output)
            gender_label = gender_labels[gender_id]
            gender_confidence = gender_output[0, gender_id]
